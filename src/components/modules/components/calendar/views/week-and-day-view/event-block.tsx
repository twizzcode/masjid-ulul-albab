...
import { formatTime } from "@/components/modules/components/calendar/helpers";
...
export function EventBlock({ event, day, className }: IProps) {
	const { badgeVariant, use24HourFormat } = useCalendar();

	const eventStart = parseISO(event.startDate);
	const eventEnd = parseISO(event.endDate);

	// If day is provided, clip the event duration to that day
	...
	if (day) {
		...
		effectiveStart = eventStart < dayStart ? dayStart : eventStart;
		effectiveEnd = eventEnd > dayEnd ? dayEnd : eventEnd;
	}

	const durationInMinutes = differenceInMinutes(effectiveEnd, effectiveStart);
	const heightInPixels = (durationInMinutes / 60) * 30 - 8;
		...
		<EventDetailsDialog event={event}>
			...
			>
				...
				{durationInMinutes > 25 && (
					<p>
						{formatTime(effectiveStart, use24HourFormat)} -{" "}
						{formatTime(effectiveEnd, use24HourFormat)}
					</p>
				)}
			</div>
		</EventDetailsDialog>
